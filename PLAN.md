...
2. Implement the bridge with `/health`, `/api/actions`, `/api/orca/version`, `/api/slice/dry-run`, `/api/slice/export-preflight`, `/api/voices/azure/en`, `/api/chat/message`, and proof ledger writes.
3. Implement the dark floating panel with hide, drag, resize, chat, quick actions, voice assignment, and proof drawer.
4. Add a Hermes Agent plugin wrapper that registers `slicer_bridge` under the `hermes_orca` toolset.
