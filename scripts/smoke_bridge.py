...
def main() -> int:
    checks = [
        ...
        ("printer_targets", request("/api/printers/targets")),
        ("printer_observe", request("/api/printers/observe", "POST", {})),
        ("printer_safety_state", request("/api/printers/safety-state", "POST", {"target_id": "flsun_t1_1"})),
        ("printer_hard_stop_proof", request("/api/printers/hard-stop-proof", "POST", {"target_id": "flsun_t1_1"})),
        ...
        ("dispatch_printer_observe", request("/api/action", "POST", {"action": "printer.observe"})),
        ("dispatch_printer_safety", request("/api/action", "POST", {"action": "printer.safety_state", "payload": {"target_id": "flsun_t1_1"}})),
